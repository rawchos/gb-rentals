...
* [`src/gb_rentals/`](src/gb_rentals/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/gb_rentals/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `gb-rentals` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/gb_rentals/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns gb-rentals.example
  (:require [gb-rentals.config :as config])
